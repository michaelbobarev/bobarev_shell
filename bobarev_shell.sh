...
TS_EOF
    ...
    if [ -n "$authkey" ]; then
        ...
        # Убрали silent_run, чтобы Tailscale смог напечатать ссылку на экран!
...
# ------------------------------------------------------------------------------
# ФУНКЦИИ ВВОДА ЧЕРЕЗ WHIPTAIL (Безопасность + UX)
# ------------------------------------------------------------------------------
...
prompt_yn() {
    ...
    if [ "$default_yes" = "true" ]; then
        if whiptail --title "Подтверждение" --yesno "$prompt" 10 60; then return 0; else return 1; fi
    else
        if whiptail --title "Подтверждение" --yesno "$prompt" 10 60 --defaultno; then return 0; else return 1; fi
    fi
}
...
prompt_default() {
    ...
    local var_name="$3"
    local val
    val=$(whiptail --title "Ввод данных" --inputbox "$prompt" 10 60 "$default_val" 3>&1 1>&2 2>&3)
    if [ $? -eq 0 ]; then
...
prompt_clean() {
    ...
    local var_name="$2"
    local val
    val=$(whiptail --title "Ввод данных" --inputbox "$prompt" 10 60 3>&1 1>&2 2>&3)
    if [ $? -eq 0 ]; then
...
mod_user_setup() {
    ...
    local pass
    pass=$(whiptail --title "Пароль" --passwordbox "Введите новый пароль для $username (оставьте пустым для отмены):" 10 60 3>&1 1>&2 2>&3)
    if [ $? -ne 0 ] || [ -z "$pass" ]; then
        echo -e "${C_YELLOW}ℹ️ Смена пароля пропущена.${C_RESET}"
    else
        local pass_confirm
        pass_confirm=$(whiptail --title "Подтверждение" --passwordbox "Повторите пароль:" 10 60 3>&1 1>&2 2>&3)
...
mod_ssh_key() {
    ...
    if [ -z "$pubkey" ]; then
        pubkey=$(whiptail --title "Ввод ключа" --inputbox "Вставьте ваш публичный ключ безопасности (ssh-ed25519, ssh-rsa и т.д.):" 10 60 3>&1 1>&2 2>&3)
    ...
    if [ -n "$pubkey" ]; then
        local mode_choice
        mode_choice=$(whiptail --title "Параметры сохранения" --menu "Выберите действие:" 12 60 2 \
...
mod_ssh_config() {
    ...
    while true; do
        ...
        if [ -z "$port" ]; then
            whiptail --msgbox "Ошибка: Значение порта должно состоять только из цифр. Попробуйте снова." 8 40
            continue
        fi
    ...
    if prompt_yn "Отключить авторизацию по паролю (разрешить доступ ТОЛЬКО по привязанным ключам)?" "false"; then
        ...
        if [ "$has_any_key" = true ]; then
            pass_auth_val="no"
            kbd_auth_val="no"
            echo -e "${C_GREEN}✅ Привязанные ключи найдены. Вход по паролю будет отключен.${C_RESET}"
        else
            whiptail --msgbox "Внимание: На сервере не найдено ни одного привязанного ключа безопасности!\nВход по паролю ОСТАЕТСЯ ВКЛЮЧЕННЫМ для предотвращения полной потери доступа." 10 60
...
mod_lock_root() {
    ...
    local root_choice
    root_choice=$(whiptail --title "Системный доступ" --menu "Выберите желаемое действие:" 12 60 2 \
    "1" "Заблокировать" \
...
mod_tailscale() {
    ...
    while true; do
        local ts_choice
        ts_choice=$(whiptail --title "Настройка закрытой сети" --menu "Управление сетевым узлом:" 18 75 9 \
        "1" "🔑 Авторизация" \
        ...
        case "$ts_choice" in
            1)
                ...
                ;;
            2)
                local en_choice=$(whiptail --title "Настройка шлюза" --menu "Действие:" 12 60 4 "1" "Назначить этот сервер шлюзом" "2" "Отключить трансляцию шлюза" "3" "Подключиться к удаленному шлюзу" "4" "Отключиться от шлюза" 3>&1 1>&2 2>&3)
                if [ $? -eq 0 ]; then
                    case "$en_choice" in
...
# 13. Менеджер Swap
mod_swap_manager() {
    local swap_choice=$(whiptail --title "Файл подкачки (Swap)" --menu "Управление виртуальной памятью:" 12 60 3 \
...
# 17. Полный аудит сервера (Компактный и интеллектуальный)
mod_server_audit() {
    clear
    echo -e "${C_CYAN}=================================================================${C_RESET}"
    echo -e "${C_BOLD}       🔍  ИНТЕЛЛЕКТУАЛЬНЫЙ АУДИТ И СОСТОЯНИЕ СИСТЕМЫ     ${C_RESET}"
    echo -e "${C_CYAN}=================================================================${C_RESET}"
    ...
    # Проверка UFW
    local ufw_st="${C_RED}Отключен${C_RESET}"
    if command -v ufw &>/dev/null && ufw status | grep -q "active"; then
        ufw_st="${C_GREEN}Активен${C_RESET}"
    fi

    # Проверка Tailscale
    local ts_st="${C_YELLOW}Не установлен${C_RESET}"
    local ts_ip="N/A"
    if command -v tailscale &>/dev/null; then
        ts_ip=$(tailscale whoami 2>/dev/null | grep -i "^  Addresses:" | grep -oP '100\.\d+\.\d+\.\d+' | head -n 1 || echo "Не подключен")
        ts_st="${C_GREEN}Активен ($ts_ip)${C_RESET}"
    fi

    # Проверка логирования
    local journal_st="${C_YELLOW}Включено${C_RESET}"
    local journal_storage=$(grep -E "^\s*Storage=" /etc/systemd/journald.conf 2>/dev/null | cut -d= -f2 || echo "auto")
    [ "$journal_storage" = "none" ] && journal_st="${C_GREEN}Отключено (Экономия диска)${C_RESET}"

    # Проверка Swap
    local swap_st=$(swapon --show --noheadings 2>/dev/null | awk '{print $1 " (" $3 ")"}' | paste -sd ", " - || echo "Отключен")
    [ -z "$swap_st" ] && swap_st="Отключен"

    # Вывод компактного отчета
    echo -e " 💻 ${C_BOLD}Хост:${C_RESET}          $(hostname) ($SYSTEM_TYPE)"
    echo -e " 🌐 ${C_BOLD}Часовой пояс:${C_RESET}  $current_tz"
    echo -e " 👤 ${C_BOLD}Администратор:${C_RESET} $active_user"
    echo -e "${C_CYAN}-----------------------------------------------------------------${C_RESET}"
    echo -e " 🔒 ${C_BOLD}Порт SSH:${C_RESET}      $ssh_port"
    echo -e " 🚫 ${C_BOLD}Вход root по SSH:${C_RESET} $( [ "$root_ssh" = "no" ] && echo -e "${C_GREEN}Запрещен${C_RESET}" || echo -e "${C_RED}Разрешен${C_RESET}" )"
    echo -e " 🔑 ${C_BOLD}Вход по паролю:${C_RESET}   $( [ "$pass_auth" = "no" ] && echo -e "${C_GREEN}Отключен (Только ключи)${C_RESET}" || echo -e "${C_YELLOW}Включен${C_RESET}" )"
    echo -e "${C_CYAN}-----------------------------------------------------------------${C_RESET}"
    echo -e " 🧱 ${C_BOLD}Брандмауэр UFW:${C_RESET} $ufw_st"
    echo -e " 🔗 ${C_BOLD}Сеть Tailscale:${C_RESET} $ts_st"
    echo -e " 🧹 ${C_BOLD}Журналы ОС:${C_RESET}    $journal_st"
    echo -e " 💾 ${C_BOLD}Файл подкачки:${C_RESET}  $swap_st"
    echo -e "${C_CYAN}=================================================================${C_RESET}"
}
